...
export default function LecturerDashboard() {
  ...
  if (loading) {
    return (
      <div className="p-10 text-center bg-gray-50 dark:bg-gray-900 min-h-screen">
    ...
    <div className="flex flex-col gap-6 p-4 w-full bg-gray-50 dark:bg-gray-900 min-h-screen" dir="rtl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-white dark:bg-gray-800 p-5 rounded-2xl shadow-sm border-r-4 border-green-500">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-gray-500 dark:text-gray-400 text-sm">מבחנים פעילים</p>
              <p className="text-3xl font-bold text-gray-900 dark:text-white">{activeExams.length}</p>
            </div>
        ...
        <div className="bg-white dark:bg-gray-800 p-5 rounded-2xl shadow-sm border-r-4 border-blue-500">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-gray-500 dark:text-gray-400 text-sm">מבחנים קרובים</p>
              <p className="text-3xl font-bold text-gray-900 dark:text-white">{upcomingExams.length}</p>
            </div>
        ...
        {/* Past Exams */}
        <div className="bg-white dark:bg-gray-800 p-5 rounded-2xl shadow-sm border-r-4 border-gray-500">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-gray-500 dark:text-gray-400 text-sm">מבחנים שהסתיימו</p>
              <p className="text-3xl font-bold text-gray-900 dark:text-white">{pastExams.length}</p>
            </div>
      ...
      {activeExams.length > 0 && (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-xl font-bold text-gray-900 dark:text-white">מבחנים פעילים כעת</h2>
          </div>
          <div className="grid gap-4">
            {activeExams.map((exam) => (
              <div
                key={exam._id}
                className="bg-green-50 dark:bg-green-900 p-5 rounded-xl border-2 border-green-200 dark:border-green-700 hover:shadow-md transition-shadow"
              >
                <div className="flex justify-between items-start mb-3">
                  <div>
                    <h3 className="text-xl font-bold text-gray-900 dark:text-white">{exam.courseName}</h3>
                    <p className="text-sm text-gray-600 dark:text-gray-400">קוד: {exam.courseCode}</p>
                  </div>
      ...
      {upcomingExams.length > 0 && (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
          <div className="flex items-center gap-2 mb-4">
            <Calendar className="text-blue-600 dark:text-blue-400" size={24} />
            <h2 className="text-xl font-bold text-gray-900 dark:text-white">מבחנים קרובים</h2>
          </div>
          <div className="grid gap-4 md:grid-cols-2">
            {upcomingExams.map((exam) => (
              <div
                key={exam._id}
                className="bg-white dark:bg-gray-700 p-5 rounded-xl border border-gray-200 dark:border-gray-600 hover:shadow-md transition-shadow"
              >
                <div className="flex justify-between items-start mb-3">
                  <div>
                    <h3 className="text-lg font-bold text-gray-900 dark:text-white">{exam.courseName}</h3>
                    <p className="text-xs text-gray-500 dark:text-gray-400">קוד: {exam.courseCode}</p>
                  </div>
                  <span className="bg-blue-50 dark:bg-blue-900 text-blue-600 dark:text-blue-400 px-2 py-1 rounded text-xs font-bold">
                    מתוכנן
                  </span>
                </div>
      ...
      {pastExams.length > 0 && (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
          <div className="flex items-center gap-2 mb-4">
            <FileText className="text-gray-600 dark:text-gray-400" size={24} />
            <h2 className="text-xl font-bold text-gray-900 dark:text-white">מבחנים שהסתיימו</h2>
          </div>
          <div className="grid gap-3">
            {pastExams.map((exam) => (
              <div
                key={exam._id}
                className="bg-gray-50 dark:bg-gray-700 p-4 rounded-xl border border-gray-200 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors cursor-pointer"
                onClick={() => window.location.href = `/exam-reports/${exam._id}`}
              >
                <div className="flex justify-between items-center">
                  <div>
                    <h3 className="font-bold text-gray-900 dark:text-white">{exam.courseName}</h3>
      ...
      {/* Empty State */}
      {exams.length === 0 && (
        <div className="bg-white dark:bg-gray-800 p-12 rounded-2xl border border-dashed border-gray-300 dark:border-gray-600 text-center">
          <BookOpen className="mx-auto text-gray-300 dark:text-gray-600 mb-4" size={48} />
          <p className="text-gray-500 dark:text-gray-400">אין מבחנים רשומים</p>
        </div>
      )}
    </div>
  );
