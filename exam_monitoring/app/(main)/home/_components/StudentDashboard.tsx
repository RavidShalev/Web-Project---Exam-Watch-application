                ...
                {upcomingExams.map((exam) => (
                    <div key={exam._id} className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
                        ...
                        <div className="mt-3 space-y-2 text-gray-600">
                            <div className="flex items-center gap-2"><Calendar size={16} className="text-gray-500"/> {exam.date}</div>
                            <div className="flex items-center gap-2"><Clock size={16} className="text-gray-500"/> {exam.startTime}</div>
                            <div className="flex items-center gap-2"><MapPin size={16} className="text-gray-500"/> {exam.location}</div>
                        </div>
                    </div>
                ))}
            </div>
      ...
      {pastExams.length > 0 && (
          <div className="mt-4 pt-6 border-t border-gray-200">
            <h3 className="text-lg font-semibold mb-3 text-gray-500 flex items-center gap-2">
                <CheckCircle size={20} className="text-green-500" /> היסטוריית מבחנים
            </h3>
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                ...
                                                {exam.rules.filter(r => !r.allowed).map((rule) => (
                                                    <span key={rule.id} className="flex items-center gap-1 bg-red-50 text-red-700 px-2 py-1 rounded text-xs border border-red-100 opacity-70">
                                                        <AlertCircle size={12} className="text-red-500"/> לא {rule.label}
                                                    </span>
                                                ))}
                                            </div>
                                        </div>
